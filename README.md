...
This repository contains the Mastering Liferay Pages and Navigation course materials.
...
1. Open your terminal and run this command according to your operating system:
   ...
   **Linux/Unix**:
   ...
   ```bash
   /bin/bash -c "$(curl -fsSL https://raw.github.com/liferay/liferay-enablement-script-library/main/content-manager-course-setup.sh)" -- --pages-navigation linux
   ```

   **Mac**:
   ```bash
   /bin/bash -c "$(curl -fsSL https://raw.github.com/liferay/liferay-enablement-script-library/main/content-manager-course-setup.sh)" -- --pages-navigation mac
   ```

   **Windows**:
   ```bash
   powershell Set-ExecutionPolicy Bypass -Scope Process -Force; iex "& { $(irm https://raw.githubusercontent.com/liferay/liferay-enablement-script-library/refs/heads/main/content-manager-course-setup.ps1); install-course --pages-navigation }"
   ```
...
2. Once the "Liferay bundle initialized" message displays, verify the `liferay-course-pages-navigation/` folder was created.

1. Go to the workspace's root folder in your terminal:

   ```bash
   cd liferay-course-pages-navigation/
   ```
...
1. Open your terminal and clone the training workspace to your computer:
   ...
   ```bash
   git clone https://github.com/liferay/liferay-course-pages-navigation
   ```
